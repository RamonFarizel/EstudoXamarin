...
namespace EstudoXamarin.iOS.Renderers
{
    ...
    public class TransitionNavigationPageRenderer : NavigationRenderer
    {
        ...
        {
            if (_transitionType == TiposDeTransicaoEnum.None)
            {
                base.PushViewController(viewController, false);
                return;
            }
            else if (_transitionType == TiposDeTransicaoEnum.Default)
            {
                base.PushViewController(viewController, animated);
                return;
            }
            else if (_transitionType == TiposDeTransicaoEnum.Fade)
            {
                FadeAnimation(View);
            }
            else if (_transitionType == TiposDeTransicaoEnum.Flip)
            {
                FlipAnimation(View);
            }
            else if (_transitionType == TiposDeTransicaoEnum.Scale)
            {
                ScaleAnimation(View);
            }
            else
        ...
        public override UIViewController PopViewController(bool animated)
        {
            ...
            if (_transitionType == TiposDeTransicaoEnum.Default)
            {
                return base.PopViewController(animated);
            }
            if (_transitionType == TiposDeTransicaoEnum.Fade)
            {
                FadeAnimation(View);
            }
            else if (_transitionType == TiposDeTransicaoEnum.Flip)
